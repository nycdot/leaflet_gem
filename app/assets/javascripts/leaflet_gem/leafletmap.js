 ...
 */
function add_click_handler(event, handler) {
	LMmap.on(event, handler);	
};
/*
 * Removes a click handler from the map
 */
function remove_click_handler(event, handler) {
...
function addMarkers(arr) {
	...
	for(var i=0;i < arr.length;i++){
        ...
        marker = createMarker(id, lat, lng, iconClass, popupText, title, open, draggable, zindex);
 ...
 */
function createMarker(id, lat, lng, iconClass, popupText, name, open, draggable, zindex) {
